/* ===== ADMIN DE INVÍTAME · parte 2 de 4 · el dibujo del panel =========================
  ...
  function triviaHtml(){
    ...
    const qs=D.trivia.map((p,i)=>{
      const opts=(p.o||[]).map((o,j)=>
        ...
        (p.o.length>2?'<span class="tqx" onclick="tqDelOpt('+i+','+j+')">'+ICO.equis+'</span>':'')+'</div>'
      ).join('');
      return '<div class="tqcard"><div class="tqhead"><b>Pregunta '+(i+1)+'</b><span class="tqx" onclick="tqDel('+i+')">'+ICO.tacho+' quitar</span></div>'+
    ...
    return '<div class="mejoras"><div class="h">'+ICO.destello+' Preguntas y respuestas de la trivia</div>'+
  ...
  function verOrden(){
    ...
    box.innerHTML='<div style="background:#fff;max-width:440px;width:100%;border-radius:14px;padding:20px;font-family:Nunito">'+
      '<h3 style="margin:0 0 4px;color:#6D1233;font-weight:900">'+ICO.ordenar+' Orden de las secciones</h3>'+
      '<div class="hint" style="margin-bottom:12px">Arrastrá, o usá las flechitas, para cambiar el orden. La portada, el pase, la confirmación y el cierre quedan fijos.</div>'+
      '<div id="ordlist"></div>'+
  ...
  function ordRender(){
    el('ordlist').innerHTML=D.secOrden.map((k,i)=>
      ...
      '<span style="cursor:grab;opacity:.6">'+ICO.agarre+'</span><span style="flex:1">'+(i+1)+'. '+(SECLABELS[k]||k)+'</span>'+
      '<button class="ordb" onclick="ordMove('+i+',-1)">'+ICO.sube+'</button><button class="ordb" onclick="ordMove('+i+',1)">'+ICO.baja+'</button></div>').join('');
  }
  ...
  function verMesas(){
    ...
    box.innerHTML='<div style="background:#fff;max-width:940px;width:100%;border-radius:14px;padding:20px;font-family:Nunito">'+
      '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px"><h3 style="margin:0;color:#6D1233;font-weight:900">'+ICO.mesa+' Gestión de mesas</h3>'+
  ...
  function galeriaHtml(){
    D.galeria=D.galeria||[];
    const thumbs=D.galeria.map((u,i)=>'<div class="gthumb"><img src="'+u+'"><span class="gx" onclick="galDel('+i+')">'+ICO.equis+'</span></div>').join('');
    return '<div class="mejoras"><div class="h">'+ICO.destello+' Galería de fotos (sin límite)</div>'+
      ...
      '<div class="file" style="margin-top:10px" id="galbtn" onclick="this.nextElementSibling.click()">'+ICO.subir+' Agregar foto(s) a la galería</div>'+
  ...
  function extrasHtml(){
    ...
    return ''+
    '<div class="mejoras"><div class="h">'+ICO.nota+' Música de fondo</div>'+
     '<div class="hint" style="margin-bottom:8px">Arranca cuando el invitado abre el sobre; queda un botón '+ICO.volumen+' para silenciar. Subí un audio o pegá el link (.mp3).</div>'+
     '<div class="file" id="audiobtn" onclick="this.nextElementSibling.click()">'+ICO.subir+' Subir audio (.mp3)</div>'+
    ...
    '<div class="mejoras"><div class="h">'+ICO.destello+' Fondo con movimiento (frase principal)</div>'+
  ...
  async function subirAudio(input){
    ...
    try{const url=await INV.uploadVideo(f); setB('musicaUrl',url); if(b)b.innerHTML=ICO.tilde+' Audio subido';}
    catch(e){ if(b)b.innerHTML=prev||(ICO.subir+' Subir audio (.mp3)'); alert('No se pudo subir: '+(e.message||e)); }
    input.value='';
  }
  ...
  function efectosHtml(){
    ...
    return '<div class="mejoras"><div class="h efx">'+ICO.destello+' Efectos — sobre de entrada</div>'+
        ...
        (_esCarta
          ? '<div class="grp"><label>Sobre del catálogo</label>'+
            ...
            '<div class="grp"><label>Color de la carta (se multiplica sobre el papel, conserva el relieve)</label>'+
              ...
              '<div class="hint">'+ICO.alerta+' El sobre de entrada NO se ve en esta vista previa (acá la invitación abre directo). Para verlo, usá el botón de ver y abrí la invitación de verdad.</div>'+
            '</div>'
          : '')+
      ...
      '</div>'+
        ...
        '<div class="hint" style="margin-bottom:10px">El calendario aparece justo después del sector de la fecha. Miralo con el botón de ver: en la vista previa de arriba no se ve.</div>'+
        (function(){ D.fx=D.fx||{};
      ...
      '<div class="mejoras"><div class="h efx">'+ICO.destello+' Adorno de las secciones</div>'+
        ...
        '</div>'+
      '</div>'+
      '<div class="hint" style="margin-top:2px">Los efectos se ven en la invitación real. Tocá <b>Guardar y publicar</b> y abrí la invitación para verlos.</div>';
  }
