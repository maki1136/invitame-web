/* ===== CÓMO SE MUESTRA LA FECHA ===============================================
   ...
   ⚠️⚠️ 14/9/2026 — ESTE ARCHIVO ESTUVO ROTO MEDIO DÍA Y FUE CULPA MÍA.
   El commit que arreglaba los círculos ovalados subió el archivo con la palabra
   «null» adentro en vez del contenido: 4 bytes. Las nueve disposiciones
   quedaron fuera de producción sin dar un solo error en consola, porque `null`
   suelto es JavaScript válido. La lección, escrita acá para no repetirla:
   después de subir un archivo por el MCP, VOLVER A LEERLO y comparar el tamaño
   con el de antes. Un archivo de 17 KB que pasa a pesar 4 bytes se ve de lejos;
   un «se subió bien» no se ve de ninguna.
   ============================================================================ */
(function () {
  'use strict';
  ...
  var CSS = [
    ...
    /* 9 · tres círculos
       ...
       ⚠️ EL ALTO VA EN AUTOMÁTICO CON aspect-ratio:1, NUNCA FIJO.
       Con height:70px fijo, cuando los tres círculos no entraban a lo ancho
       (marco de 292 px, o cualquier pantalla por debajo de ~380 px) el flex
       achicaba el ancho y dejaba el alto quieto: 48 × 76. Y border-radius:50%
       sobre una caja que no es cuadrada dibuja una ELIPSE, no un círculo.
       Medido con la invitación real embebida en dos anchos distintos.
       flex:0 1 70px + min-width:0 es lo que le da permiso a encogerse parejo. */
    '.ivf-circ{gap:12px}',
    '.ivf-circ .c{position:relative;width:70px;height:auto;aspect-ratio:1;',
    '  flex:0 1 70px;min-width:0;border-radius:50%;overflow:hidden;',
