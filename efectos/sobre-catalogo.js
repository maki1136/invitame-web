  ...
  function actualizar(m, id) {
    ...
    if (env.dataset.apertura === 'solapas') {
      ...
      /* ★★ AL PASAR A UN SOBRE DE VIDEO HAY QUE DESARMAR EL DE SOLAPAS.  22/9/2026
         ...
         por `cantera-video`: el <video> tomaba el archivo nuevo, pero la caja
         `#col-sobre-foto` que había armado `montarSolapas` seguía colgada de #env,
         con las cuatro hojas del sobre VIEJO pintadas inline, y la lista blanca de
         `#env.carta-video` la deja visible: se veía el sobre anterior arriba del
         video. Esta rama nunca desarmaba nada. */
      var solapasViejas = document.getElementById('col-sobre-foto');
      if (solapasViejas && solapasViejas.parentNode) {
        solapasViejas.parentNode.removeChild(solapasViejas);
      }
